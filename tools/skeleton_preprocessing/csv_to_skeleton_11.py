...
# dirname
today = str(date.today()).replace("-","")[2:] + "_errorlist"
#bvh list있는 경로
tmp='Updated_bvhlist/bvh_list_11.txt' # <<수정필요>>

# 저장 폴더경로
# out_path='./'+today+'_preprocessed' # skeleton 파일 저장 경로
out_path='./Updated_skeletons' # skeleton 파일 저장 경로
                ...
                with open(json_file,'r',encoding='utf8') as j:
                    try:
                        ...
                        try:
                            ...
                            with open(today+".txt", "a") as error_file:
                                data = file + ": " + "json컬럼에러" + "\n"
